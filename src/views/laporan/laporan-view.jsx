          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-7 gap-2">
              {getDaysInMonth(currentMonth).map((day, index) => {
                ...
                const totalCalories = foodData ? getTotalCalories(foodData.foods) : 0;
                const calorieStatus = totalCalories > 0 ? getCalorieStatus(totalCalories) : null;
                    ...
                    {day && (
                      <>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          {calorieStatus && (
                            <span className={`text-xs px-1.5 py-0.5 rounded-full ${calorieStatus.color}`}>
                        ...
                        {foodData && (
                          <div className="space-y-1">
                            ...
                            <div className="grid grid-cols-3 gap-1">
                              {foodData.foods.slice(0, 6).map((food, foodIndex) => (
                                <div
                                  key={foodIndex}
                                ...
                                >
                                  <img
                                    ...
                                    onError={(e) => {
                                      e.target.src = "data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjQiIGhlaWdodD0iMjQiIHZpZXdCb3g9IjAgMCAyNCA0IiBmaWxsPSJub25lIiB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPgo8cmVjdCB3aWR0aD0iMjQiIGhlaWdodD0iMjQiIGZpbGw9IiNGM0Y0RjYiLz4KPHA+"
                                    }}
                                  />
                                  {/* Category indicator */}
                                  <div className={`absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full border border-white ${
                                    food.category === 'Sarapan' ? 'bg-yellow-400' :
                                    food.category === 'Makan Siang' ? 'bg-orange-400' :
                                    'bg-purple-400'
                                  }`}></div>
                                </div>
                              ))}
                              {foodData.foods.length > 6 && (
                                <div className="w-6 h-6 bg-gray-200 rounded border flex items-center justify-center">
                                  <span className="text-xs text-gray-600">+{foodData.foods.length - 6}</span>
                                </div>
                              )}
                            </div>
        ...
        {selectedDate && getFoodDataForDate(selectedDate) && (
          <div className="mt-6 bg-white rounded-xl shadow-sm border p-6">
            <h3 className="text-lg font-bold text-gray-900 mb-4">
              Detail Makanan - {selectedDate} {monthNames[currentMonth.getMonth()]} {currentMonth.getFullYear()}
            </h3>
              ...
              {getFoodDataForDate(selectedDate).foods.map((food, index) => (
                <div key={index} className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500">{food.time} • {food.category}</p>
                    <p className="text-sm font-medium text-orange-600">{food.calories} kalori</p>
                  </div>
                </div>
              ))}
            </div>
